...
fn get_exe_modified() -> Option<SystemTime> {
    std::env::current_exe()
        .ok()
...
/// Updates files when the executable is newer than the extracted file
fn extract_embedded_devices() -> Result<(), String> {
    ...
    let exe_modified = get_exe_modified();
    ...
    for (filename, content) in EMBEDDED_DEVICES {
        ...
        // Check user path — update if executable is newer
        else if user_path.exists() {
            let needs_update = match (&exe_modified, fs::metadata(&user_path).ok().and_then(|m| m.modified().ok())) {
                (Some(exe_time), Some(file_time)) => exe_time > &file_time,
                _ => false, // If we can't determine timestamps, don't overwrite
            };
            if needs_update {
                fs::write(&user_path, content)
                    .map_err(|e| format!("Failed to update device file '{}': {}", filename, e))?;
                warn_log!("Updated device file: {}", filename);
                created_count += 1;
            } else {
                verbose_log!("Device file up to date in user path: {}", filename);
                skipped_user += 1;
            }
        }
        // If neither exists, create in user path
            ...
            warn_log!("Installing device file: {}", filename);
            created_count += 1;
        }
    }

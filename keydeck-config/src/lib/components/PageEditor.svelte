...
<script lang="ts">
  ...
  function updateWhen(when: any) {
    if (!page) return;
    if (when) {
      page.when = when;
      config[groupKey][pageName].when = when;
    } else {
      delete page.when;
      delete config[groupKey][pageName].when;
    }
  }
...
<div class="page-editor">
  ...
  <div class="section">
    <div class="form-group">
      <WhenEditor when={page?.when} resetKey={pageName} onUpdate={updateWhen} />
    </div>
